    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          ...
          "For who the agent is, sign the requests. Web Bot Auth does this with cryptographic signatures on the requests themselves, built on an existing standard for signing them (RFC 9421): a Signature header, a Signature-Agent header pointing at a directory of public keys, and a Signature-Input header saying what was signed. It rests on two drafts at the IETF, the body that standardises internet protocols — one covering the directory, one the protocol itself — so it is a live proposal rather than a settled standard. It is also not theoretical. OpenAI signs the outbound requests from ChatGPT's cloud browser this way and publishes its verification keys at a well-known directory. Cloudflare put forward a registry format in February 2026, and the registry work has since continued as a draft authored jointly with Amazon.",
